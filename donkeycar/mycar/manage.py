...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    ### add ultrasonic 20240731HK
    if cfg.HAVE_ULTRASONIC:
        #from donkeycar.parts.ultrasonic import Ultrasonic_donkeycar
        import os
        import sys
        togikaidrive_path = os.path.join(os.path.dirname(sys.path[0]), "togikaidrive")
        sys.path.append(togikaidrive_path)
    ...
    ### add for ultrasonic 20240731HK
        ...
        inputs = ['cam/image_array', 'ultrasonic/dist_array', 'user/angle', 'user/throttle', 'user/mode']
        types = ['image_array', 'list','float', 'float', 'str']
    ### add for ultrasonic 20240731HK
    else:
